...
def _validateValues(values):
    if not isinstance(values, Base):
        msg = 'knownValues and predictedValues must be derived class of ' \
              'nimble.data.Base'
        raise InvalidArgumentType(msg)

    if len(values.features) > 1 and len(values.points) > 1:
        msg = ('knownValues and predictedValues must be of the form 1xn '
               'or nx1. Only objects with either one feature for multiple '
               'points or one point for multiple features.')
...
def _validateNumOfElements(knownValues, predictedValues):
    if len(knownValues) != len(predictedValues):
        msg = "knownValues and predictedValues must have the same " \
              "number of elements"
...
def meanSquareLoss(knownValues, predictedValues):
    """
    Compute the mean square error.  Assumes that knownValues and
    predictedValues contain numerical values, rather than categorical data.
    """
    try:
        ...
        raise ZeroDivisionError(
            'Tried to divide by zero when calculating metric. '
            'predictedValues size is 0')
...
def meanSquareLoss_alt(knownValues, predictedValues):
    """
    Compute the mean square error based on sklearn-learn mean_squared_error.
    Assumes that knownValues and predictedValues contain numerical values,
...
def sumSquareLoss(knownValues, predictedValues):
    """
    Compute the sum square error.  Assumes that knownValues and predictedValues
    contain numerical values, rather than categorical data.
    """
...
def crossEntropyLoss(knownValues, predictedValues, eps=1e-15):
    """
    Compute the cross-entropy loss (log-loss).  Assumes that knownValues and
    predictedValues contain numerical values, rather than categorical data.
    ...
    if predictedValues.all() > 1 or predictedValues.all() < 0:
        msg = ('predictedValues for cross entropy loss involved values from a '
               'logistic regression model correspond to probabilities '
               'between 0 and 1.')
        raise InvalidArgumentValue(msg)
...
def crossEntropyLoss_alt(knownValues, predictedValues):
    """
    Compute the cross-entropy loss based on sklearn-learn log_loss function.
    Assumes that knownValues and predictedValues contain numerical values,
    rather than categorical data.
    """
    knownValues, predictedValues = _formatInputs(knownValues,
...
def exponentialLoss(knownValues, predictedValues, tau):
    """
    Compute exponetial loss. Assumes that knownValues and
    predictedValues contain numerical values, rather than categorical data.
    """
    ...
    try:
        ...
        msg = ('Exponent computation not possible. The function works '
               'better when values between 0 and 1 are given. '
               'Also, changing tau value could help')
...
def quadraticLoss(knownValues, predictedValues):
    """
    Compute quadratic loss. Assumes that knownValues and
    predictedValues contain numerical values, rather than categorical data.
    """
    summatory = sumSquareLoss(knownValues, predictedValues)
    try:
...
def hingeLoss_alt(knownValues, predictedValues):
    """
    Compute hinge loss based on sklearn-learn hinge_loss function.
    Assumes that knownValues and predictedValues contain numerical values,
    rather than categorical data.
    """
    knownValues, predictedValues = _formatInputs(knownValues,
...
def meanAbsoluteError(knownValues, predictedValues):
    """
    Compute mean absolute error. Assumes that knownValues and predictedValues
    contain numerical values, rather than categorical data.
    """
    try:
...
def meanFeaturewiseRootMeanSquareError(knownValues, predictedValues):
    ...
    if len(knownValues.features) != len(predictedValues.features):
        raise InvalidArgumentValueCombination(
            "The known and predicted data must have the "
...
def varianceFractionRemaining(knownValues, predictedValues):
    ...
    if len(knownValues.features) != len(predictedValues.features):
        raise Exception(
            "Objects had different numbers of features. Known values had " +
            str(len(knownValues.features)) + " and predicted values had " +
            str(len(predictedValues.features)))
